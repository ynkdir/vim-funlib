...
function! s:F(x, y, z)
  return bitwise#or(bitwise#and(a:x, a:y), bitwise#and(bitwise#not(a:x), a:z))
endfunction
"#define G(x, y, z) (((x) & (z)) | ((y) & (~z)))
function! s:G(x, y, z)
  return bitwise#or(bitwise#and(a:x, a:z), bitwise#and(a:y, bitwise#not(a:z)))
endfunction
...
function! s:I(x, y, z)
  return bitwise#xor(a:y, bitwise#or(a:x, bitwise#not(a:z)))
endfunction

" ROTATE_LEFT rotates x left n bits.
"#define ROTATE_LEFT(x, n) (((x) << (n)) | ((x) >> (32-(n))))
function! s:ROTATE_LEFT(x, n)
  return bitwise#or(bitwise#lshift(a:x, a:n), bitwise#rshift(a:x, 32 - a:n))
endfunction
...
function! s:FF(a, b, c, d, x, s, ac)
  let a = a:a
  let a += s:F(a:b, a:c, a:d) + a:x + a:ac
...
function! s:GG(a, b, c, d, x, s, ac)
  let a = a:a
  let a += s:G(a:b, a:c, a:d) + a:x + a:ac
  let a = s:ROTATE_LEFT(a, a:s)
  let a += a:b
  return a
endfunction
...
function! s:HH(a, b, c, d, x, s, ac)
  let a = a:a
  let a += s:H(a:b, a:c, a:d) + a:x + a:ac
  let a = s:ROTATE_LEFT(a, a:s)
  let a += a:b
  return a
endfunction
...
function! s:II(a, b, c, d, x, s, ac)
  let a = a:a
  let a += s:I(a:b, a:c, a:d) + a:x + a:ac
  let a = s:ROTATE_LEFT(a, a:s)
  let a += a:b
  return a
endfunction
...
function! s:MD5Init(context)
  let a:context.count[0] = 0
  let a:context.count[1] = 0
  " Load magic initialization constants.
  let a:context.state[0] = 0x67452301
  let a:context.state[1] = 0xefcdab89
  let a:context.state[2] = 0x98badcfe
  let a:context.state[3] = 0x10325476
endfunction
...
function! s:MD5Update(context, input, inputLen)
  let index = bitwise#and(bitwise#rshift(a:context.count[0], 3), 0x3F)

  let a:context.count[0] += bitwise#lshift(a:inputLen, 3)
  if bitwise#cmp(a:context.count[0], bitwise#lshift(a:inputLen, 3)) < 0
    let a:context.count[1] += 1
  endif
  let a:context.count[1] += bitwise#rshift(a:inputLen, 29)

  let partLen = 64 - index

  if a:inputLen >= partLen
    call s:MD5_memcpy(a:context.buffer, index, a:input, partLen)
    call s:MD5Transform(a:context.state, a:context.buffer)

    let i = partLen
    while i + 63 < a:inputLen
      call s:MD5Transform(a:context.state, a:input[i :])
      let i += 64
    endwhile
  ...
  call s:MD5_memcpy(a:context.buffer, index, a:input[i :], a:inputLen - i)
endfunction
...
function! s:MD5Final(digest, context)
  ...
  call s:Encode(bits, a:context.count, 8)

  let index = bitwise#and(bitwise#rshift(a:context.count[0], 3), 0x3f)
  let padLen = (index < 56) ? (56 - index) : (120 - index)
  call s:MD5Update(a:context, s:PADDING, padLen)

  call s:MD5Update(a:context, bits, 8)

  call s:Encode(a:digest, a:context.state, 16)
...
function! s:Encode(output, input, len)
  ...
  let j = 0
  while j < a:len
    let a:output[j] = bitwise#and(a:input[i], 0xff)
    let a:output[j+1] = bitwise#and(bitwise#rshift(a:input[i], 8), 0xff)
    let a:output[j+2] = bitwise#and(bitwise#rshift(a:input[i], 16), 0xff)
    let a:output[j+3] = bitwise#and(bitwise#rshift(a:input[i], 24), 0xff)
    let i += 1
    let j += 4
  endwhile
endfunction
...
function! s:Decode(output, input, len)
  ...
  let j = 0
  while j < a:len
    let a:output[i] = a:input[j] + (a:input[j+1] * 0x100) +
          \ (a:input[j+2] * 0x10000) + (a:input[j+3] * 0x1000000)
    let i += 1
    let j += 4
  endwhile
endfunction
